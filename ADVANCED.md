...
每日耗气量」口径，并按账单核对**抄表间隔与阶梯用量**。重新抓包后可顺手导出一份：
...
- 能源面板如需**逐日连续计量**，请用模板传感器引用
  `上个账期累计表读数` 的属性 `estimated_reading`（= 官方读数 + 本账期累计用量）。
...
3. 2026 年账单累计 `145.0 m³` = 接口 `cycleCreditQty`（年度第一档累计量），
   全年未触达第二阶梯，与阶梯预期一致；
...
| `meterIds[].cycleCreditQty` | 年度第一档累计量（阶梯核对用） |
